  ...
  const navigationLinks = links.map((link) => {
    if (link === "Categories"){ return(
      <div 
        ...
        >
        <li 
          className={`flex transition-transform duration-100 ease-in-out group-hover:cursor-pointer ${categoryDropdown?'-translate-y-1 border-b-2':''}  border-b-primary`}
          >
              {link} 
      ...
      return (
        ...
        <Link className=" pb-2 group" key={link} to={link === 'Home'?'/':link.toLowerCase()}>
          <li
            className="transition-transform  duration-100 ease-in-out group-hover:-translate-y-1 group-hover:cursor-pointer group-hover:mb-4  group-hover:border-b-2 border-b-primary"
  ...
  return (
    <>
      ...
      <div 
          onMouseOver={() =>{ setCategoryDropdown(true)}}
          onMouseLeave={() =>{ setCategoryDropdown(false)}}
          className={`absolute bg-gradient-to-b from-zinc-100 to-[#f7f9ef] outline-[#e7edcf] left-17 top-[33px] rounded-sm shadow-sm transition-all ease-in-out duration-200 grid ${categoryDropdown?'grid-rows-[1fr] pt-4 pb-2 outline-1':'grid-rows-[0fr]'}`}>

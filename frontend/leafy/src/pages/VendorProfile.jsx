...
const VendorProfile = () => {
    ...
    const [pfp, setPfp] = useState(
        "https://i.pinimg.com/736x/d2/c0/08/d2c00886ae6f6cd0c88ee864d1023b59.jpg"
    );
    ...
    return (
        ...
        {editProfPop && (
            ...
            <div className="relative flex flex-col w-full max-w-[600px] h-full max-h-fit py-8 px-4 sm:px-8 gap-4 bg-zinc-100 rounded-xl overflow-hidden">
                <div
                onClick={() => setEditProfPop(false)}
                className="absolute top-2 right-2"
                >
                {/* <h1 className="font-bold text-2xl">Add New Address</h1> */}
                <svg
                    className={`fill-black w-8 h-8 shrink-0 hover:cursor-pointer `}
                ...
                <label className="font-semibold" htmlFor="vpfp">
                    Pfp (URL)
                </label>
                <input
                    className="py-1 px-3 outline-1 outline-zinc-400 rounded-sm"
                    name="vpfp"
                    value={vendorDetails["vpfp"]}
                ...
                <div className="flex flex-col mt-2">
                <label className="font-semibold" htmlFor="business_name">
                    Business Name
                </label>
                <input
                    className="py-1 px-3 outline-1 outline-zinc-400 rounded-sm"
                ...
                <button
                onClick={() => saveChanges()}
                className="flex justify-center bg-primary py-1 rounded-sm text-xl font-semibold hover:cursor-pointer"
                >
                Save
                </button>
            </div>
        ...
        {deletePop && (
            ...
            <div className="relative flex flex-col w-full max-w-[400px] h-full max-h-fit py-8 px-4 sm:px-8 gap-4 bg-zinc-100 rounded-xl overflow-hidden">
                ...
                <button
                    ...
                    className="py-1 px-2 bg-zinc-500 text-white font-semibold rounded-sm"
                >
                    Cancel
                </button>
                <button
                    onClick={() => confirmDelete()}
                    className="py-1 px-2 bg-red-500 text-white font-semibold rounded-sm"
                >
                    Confirm
                </button>
                </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-4 sm:px-12 py-[1rem] ">
            <div className="mt-16  w-full h-[200px] sm:min-h-[200px] rounded-lg sm:rounded-[0px] bg-primary mb-5 px-2 sm:px-0 flex flex-col sm:flex-row justify-center sm:justify-start items-center relative top-10 sm:top-0 shadow-md">
            <div className="self-center aspect-square absolute sm:static -top-10 sm:top-0 sm:self-start h-[100px] w-[100px] sm:w-[200px] sm:h-full bg-zinc-800 rounded-[50%] sm:rounded-tl-[0%] sm:rounded-bl-[0%] overflow-hidden shrink-0">
                ...
                src={
                    (vendorDetails.vpfp == "" || vendorDetails.vpfp == null) &&
                    vendorDetails != undefined
                    ? "https://i.pinimg.com/736x/9f/e7/3f/9fe73f40dbd9713c87e785379a595ec3.jpg"
                    : vendorDetails.vpfp
                }
                alt=""
                />
            ...
            <div
                onClick={() => setEditProfPop(true)}
                className="group absolute top-0 right-0 sm:right-0 py-2 px-2 hover:cursor-pointer"
            >
                <svg
                className="h-6 w-6 fill-text transition-all ease-in-out duration-400 group-hover:rotate-90 "
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 -960 960 960"
                >
            ...
            <h1 className="pt-4 sm:pl-5 sm:pt-0 font-bold text-[clamp(2rem,7vw,5rem)] tracking-wider text-center">
                {vendorDetails != undefined && vendorDetails.business_name != ""
            ...
            <div className="relative w-full mb-5 top-10 sm:top-0">
                <Link 
                    to={'/create'}
                    className="ml-auto gap-2 py-1 px-2 bg-primary rounded-md shadow-md flex w-full mobile:w-fit text-xl font-semibold justify-center">
                    <svg className="fill-text hidden mobile:block" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill=""><path d="M440-120v-320H120v-80h320v-320h80v320h320v80H520v320h-80Z"/></svg>
                    Add a product
                </Link>
            </div>
            ...
            {prodData !== undefined &&
                ...
                return (
                    <div
                    key={prod.id}
                    className="relative shadow-lg bg-zinc-800 rounded-md flex flex-col w-full h-full flex-1"
                    >
                    <div className="w-full aspect-square overflow-hidden self-start flex items-center justify-center rounded-t-md shrink-0">
                        <img
                        className="object-cover object-center w-full shrink-0"
                    ...
                    <div className="pt-5 p-4 justify-between flex flex-col flex-1 text-white">
                        ...
                        <h2 className="font-bold text-3xl pb-2">
                            <a
                            onClick={() => handleCardClick(prod.id, prod.title)}
                            href=""
                            >
                        ...
                        <div className="text-[0.8rem] pb-2 flex items-center">
                            rating:
                        ...
                        <h2 className="relative top-1 font-bold text-4xl  min-w-[120px] ">
                            ₹ {prod.price}
                        </h2>
                        <button
                            onClick={() => handleModMenu(prod.id)}
                            className="group hover:cursor-pointer hover:scale-105 hover:font-bold transition-all ease-in-out duration-300 z-5 rounded-md items-center font-[600] text-black max-h-[40px]"
                        >
                            <svg
                            className="h-8 w-8 fill-white group-hover:fill-primary "
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 -960 960 960"
                            >
                        ...
                        <div
                            className={`absolute grid transition-all ease-in-out duration-200 ${
                            prod.modMenu ? "grid-rows-[1fr]" : "grid-rows-[0fr]"
                            }  right-6 `}
                        >
                            <div
                            className={`overflow-hidden bg-zinc-300 transition-all ease-in-out duration-200 ${
                                prod.modMenu
                                ? "px-4 py-0.5  outline-1 outline-black"
                                : ""
                            } rounded-sm`}
                            >
                            <p
                            ...
                            <p
                                className="text-black hover:cursor-pointer"
                                onClick={() => initiateDelete(prod.id)}
                            >
                                remove
                            </p>
                            </div>

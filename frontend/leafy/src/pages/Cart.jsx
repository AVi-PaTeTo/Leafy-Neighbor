...
const Cart = (props) => {
  ...
  const [toastMessage, setToastMessage] = useState("Item removed from cart.")
  ...
  const handleRemoveItem = async (id) => {
    try {
      ...
      setLoading(false);
      updateToast('Item removed from cart.')
    }
  };
  ...
  return (
    <div className="relative w-full h-screen min-h-screen ">
      <div
        className={`transition-all ease-in-out duration-200 fixed inset-0 bg-black/70  px-4 ${
          loading ? "opacity-100 z-10" : "opacity-0 z-[-10]"
        }`}
      >
      ...
      {popupVisible && (
        <div
          ...
          className=" absolute flex justify-center items-center z-55 inset-0 bg-black/60"
        >
          <div className="fixed bg-gray-100 rounded-md flex flex-col">
            <h1 className="text-center font-bold text-3xl pt-2 border-b-1 border-b-gray-300 w-full">
              Confirm Delete
            </h1>
            <h2 className="px-8 py-4 font-semibold">
              You're about to remove "{popupMessage}" from your cart
            </h2>
            <div className="pb-4 px-8 flex justify-between">
              <button
                onClick={() => handleRemoveItem(itemToRemove)}
                className="hover:bg-red-500 hover:font-semibold hover:cursor-pointer transition-all duration-300 px-4 py-1 outline-1 rounded-sm font-semibold"
              >
                Confirm
              </button>
              <button
                ...
                className="hover:cursor-pointer px-4 py-1 outline-1 rounded-sm font-semibold"
              >
                Cancel
              </button>
            </div>
      ...
      <div className="flex justify-center mt-16 md:mt-16 h-screen min-h-fit max-h-[calc(100vh-5.5rem)] w-full px-2 py-8">
        ...
        <div className="max-w-[800px] bg-white/20 shadow-2xl backdrop-blur-[4px] relative w-full min-h-full h-fit rounded-lg ">
          ...
          <div
            className={`sticky w-full h-fit justify-end bg-zinc-800 rounded-t-lg z-10 transition-all ease-in-out top-16  ${
              props.hide === true
                ? "md:top-0 duration-200"
                : "md:top-16 duration-300 delay-50"
            }`}
          >
            <div className="flex justify-between p-4">
              <div className="flex gap-2">
                <h2 className="font-semibold text-3xl pt-1 text-gray-100 tracking-wide">
                  Cart Total:{" "}
                </h2>
                <h2 className="font-semibold text-3xl pt-1 text-gray-100 tracking-wide">
                  ₹ {cartTotal}
              ...
              <button
                ...
                className="hidden hover:cursor-pointer sm:flex items-center gap-4 py-1 px-4 text-xl font-bold rounded-md bg-primary shadow-sm"
              >
              ...
              <div className="flex flex-col gap-6 pb-14 py-3 md:px-10 overflow-hidden scroll-auto">
                {cartItems && cartItems.length > 0 ? (
                  cartItems.map((item) => (
                    <span
                      key={item.id}
                      className="relative flex h-50 bg-zinc-200/70 rounded-md shadow-md"
                    >
                      ...
                      <div className="min-w-[150px] h-full">
                        <img
                          className="h-[80%] w-full mobile:h-full object-cover self-start rounded-l-md"
                        ...
                        <div className="flex mobile:hidden flex-col w-full sm:flex-col  sm:justify-end  gap-2 px-2 py-1.5 sm:gap-4 sm:pb-4 sm:px-4">
                          <button
                            onClick={() =>
                              handleRemoveButton(item.id, item.product_name)
                            }
                            className="outline-1  px-4 py-0.5  rounded-md  max-w-[180px] hover:bg-red-500 focus:bg-red-500 transition-all ease-in-out duration-300"
                          >
                            Remove
                          </button>
                        </div>
                      ...
                      <div className="flex flex-col w-full h-full">
                        <div className="flex flex-col w-full h-full justify-between">
                          <div className="px-2 sm:px-4 md:px-8 pt-4 max-w-full overflow-clip">
                            <h2 className="hover:cursor-pointer relative font-semibold text-2xl md:text-4xl">
                              {item.product_name}
                              <div
                                onClick={() =>
                                  handleCardClick(
                          ...
                          <div className="flex w-full flex-wrap">
                            <h2 className="pl-2 sm:pl-4 md:pl-8 font-semibold text-2xl md:text-3xl">
                              ₹ {item.price}
                            </h2>
                        ...
                        <div className="hidden mobile:flex flex-col w-fit md:w-full sm:flex-row self-end sm:justify-end gap-2 py-2 px-2 sm:gap-4 sm:pb-4 sm:px-4">
                          <button
                            ...
                            className="outline-1  px-[15px] py-0.5 pt-1 font-semibold rounded-md  max-w-[180px] hover:outline-red-500 focus:outline-red-500  hover:text-gray-100 focus:text-gray-100 hover:bg-red-500 focus:bg-red-500 transition-all ease-in-out duration-300"
                          >
                            Remove
                          </button>
                        </div>
                  ...
                  <div className="text-center flex flex-col items-center justify-center h-[200px] text-3xl mt-10">
                    <svg
                      className="w-[170px] h-[170px]  fill-black/50 shrink-0"
                    ...
                    <p>Hmm… no plants here yet 🌿</p>
                    <p>Go find something green to bring home!</p>
                  </div>
                )}
              </div>
              <div
                onClick={() => handlePayment()}
                className="flex hover:cursor-pointer font-semibold text-2xl text-text  sm:hidden fixed left-0 bottom-0 w-full rounded-b-lg bg-primary p-2 justify-center items-center z-75"
              >
                  Proceed to Checkout
              </div>
            </div>

...
import { patchProduct, getProductById, getCategories, getTags, getUser } from "../api/ApiFunctions";
...
const ModifyProduct = () => {
    ...
    const [tempData, setTempData] = useState({tags:[], categories:[]})
    ...
    useEffect(()=>{
        ...
        const authenticate = async() => {
            ...
            if(allow){
                ...
                setProdFormData({
                        name: prod.title || '', // 'title' from API goes to 'name'
                        price: prod.price !== undefined ? prod.price.toString() : '', // ensure string for controlled input
                        stock: prod.stock !== undefined ? prod.stock.toString() : '',
                        short_description: prod.description?.short_description || '',
                        care_instructions: prod.description?.care_instructions || '',
                        detailed_description: prod.description?.detailed_description || '',
                        });
                setFeatureIndex(prod.description?.features.length || 0)
                setFeatures(prod.description?.features.map((f,idx) => ({index: idx, text:f})) || [])
                const specList =Object.keys(prod.description?.specifications || {}).map((key, idx) => ({index: idx,
                                                                                                    key: key,
                                                                                                    value: prod.description.specifications[key]
                                                                                                    }))
                setSpecifications(specList)
                setSpecIndex(specList.length)
                
                setImages(prod.images.map(img => ({id: img.image, file: undefined})))

                setTempData({tags:prod.tags, categories: prod.categories})
                }
        }
        authenticate()
    },[])
    ...
    useEffect(() => {        
            const setCategoriesAndTags =() =>{
                ...
                for(const tg in tags){
                    for(const t in tags[tg].tags){
                        if(tempData['tags'].includes(tags[tg].tags[t].id)){
                            if(['Light Requirement', 'Watering Needs', 'Size'].includes(tags[tg].name)){
                                li = {...li, [tags[tg].name]:tags[tg].tags[t].id}
                            } else {
                                if (li['other']) {
                                    li['other'].add(tags[tg].tags[t].id);  // Add to the existing Set
                                    } else {
                                    li['other'] = new Set();
                                    li['other'].add(tags[tg].tags[t].id); // Add the id to new Set
                                }
                            }
                        }
                ...
                setSelectedCategories(tempData.categories)
                setSelectedTags(li)
            }
    ...
    const createJson = async (e) => {
        ...
        try {
            const response = await patchProduct(prodId,formData);
            ...
            <div className="pt-12 flex justify-center ">
                ...
                <form className="mt-5 p-4 sm:p-12 sm:pt-8 rounded-md w-screen max-w-[600px] flex flex-col  bg-primary ">
                    ...
                    <fieldset className="flex flex-col w-full border-2 border-text rounded-md mt-4 p-4 gap-3">
                                                ...
                                                return(
                                                        <label key={tag.id} className="hover:bg-gray-200/80 flex items-center pl-6 py-1 cursor-pointer select-none">
                                                            <input
                                                                ...
                                                                type="radio"
                                                                name={t.name}
                                                                checked={selectedTags[t.name] == tag.id}
                                                    ...
                                                    return(
                                                        <label key={tag.id} className="hover:bg-gray-200/80 flex items-center pl-6 py-1 cursor-pointer select-none">
                                                            <input
                                                                // disabled={loading}
                                                                type="checkbox"
                                                                checked={selectedTags.other ? selectedTags.other.has(tag.id) : false}
                                                                value={tag.id}
                                                                className="mr-3 accent-primary"
                                                                onChange={()=> handleTagChange(t.name, tag.id)}
                                                            />
                        ...
                        <button onClick={createJson} className="mt-8 py-2 px-6 bg-accent rounded-md text-white font-bold">Save Changes</button>
                </form>
            </div>
        </div>
    )
